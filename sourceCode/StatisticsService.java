...
public class StatisticsService {
    ...
    public static String generateStatistics(List<RecurringEvent.EventSeries> events) {
        ...
        // --- CHANGED: Set range to Current Calendar Year (Jan 1 - Dec 31) ---
        ...
        for (RecurringEvent.EventSeries event : events) {
            ...
            // Get all occurrences in the CURRENT YEAR
            List<LocalDateTime> occurrences = engine.getOccurrences(event, startOfYear, endOfYear);
        ...
        report.append("📊 EVENT STATISTICS (").append(currentYear).append(")\n");
        report.append("=".repeat(50)).append("\n\n");

        report.append("📈 Total Events Created: ").append(totalEvents).append("\n\n");
        ...
        if (busiestDay != null) {
            ...
            report.append(" (").append(maxCount).append(" occurrences in ").append(currentYear).append(")\n\n");
        }

        report.append("📅 Event Distribution by Day of Week (").append(currentYear).append("):\n");
        for (DayOfWeek day : DayOfWeek.values()) {
